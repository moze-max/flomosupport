...
class StorageService {
  ...
  static Future<String?> saveAvatar(io.File avatarFile) async {
    ...
    try {
      ...
      // 6. 使用 _fileSystem 创建新文件，并复制内容
      final File newAvatar = _fileSystem.file(newAvatarPath);
      await newAvatar
          .writeAsBytes(await avatarFile.readAsBytes()); // Copy content
      ...
      final String? oldAvatarPath = await _readCurrentAvatarPath();
      if (oldAvatarPath != null && oldAvatarPath != newAvatar.path) {
        // 7. 使用 _fileSystem 获取旧头像文件实例
        final oldAvatarFile = _fileSystem.file(oldAvatarPath);
        if (await oldAvatarFile.exists()) {
          await oldAvatarFile.delete();
          developer.log("Deleted old avatar: $oldAvatarPath");
        }
  ...
  /// Loads the saved avatar image based on the path stored in a text file.
  /// Returns a [io.File] object of the avatar, or `null` if no avatar is found.
  static Future<io.File?> loadAvatar() async {
    // Returns dart:io.File
    try {
      final String? avatarPath = await _readCurrentAvatarPath();
      if (avatarPath != null && avatarPath.isNotEmpty) {
        // 8. 从 _fileSystem 读取文件内容，然后创建一个新的 dart:io.File 返回
        ...
        if (await avatarFileInMem.exists()) {
          ...
          // 创建一个临时的 dart:io.File，并将内存文件内容写入
          final tempIoFile = io.File(
              avatarFileInMem.path); // Path might not map directly to real file
  ...
  static Future<void> deleteAvatar() async {
    try {
      final String? avatarPath = await _readCurrentAvatarPath();
      if (avatarPath != null && avatarPath.isNotEmpty) {
        // 9. 使用 _fileSystem 获取 File 实例
        final File avatarFile = _fileSystem.file(avatarPath);
        if (await avatarFile.exists()) {
          await avatarFile.delete();
          developer.log("Avatar image deleted from: $avatarPath");
        }
